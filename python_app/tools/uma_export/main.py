...
# 繁中版 master.mdb 內建繁中名稱, 不需線上翻譯
class FakeTranslator:
    def translate_support_card(self, card):
        return card
    def translate_chara_card(self, uma):
        uma.id = int(uma.id)
        return uma
...
db = Umadb(sys.argv[1] if len(sys.argv) > 1 else '../master.mdb')
...
if __name__ == "__main__":
    ...
    with open('cardDB.json', 'w', encoding='utf-8') as f:
        f.write(json.dumps(cardDB, ensure_ascii=False, indent=2))
        f.write("\n")
    with open('umaDB.json', 'w', encoding='utf-8') as f:
        f.write(json.dumps(umaDB, ensure_ascii=False, indent=2))
        f.write("\n")